...
controller.comentarios = (req, res) => {
    ...
    req.getConnection((err, conn) => {
        ...
        conn.query('INSERT INTO Comentarios (ID_Usuario, ID_Docente, ID_Materia, Nombre_Materia, Comentario, Calificacion) VALUES (?, ?, null, ?, ?, ?);',
        [req.cookies.usuario,data.ID_Docente,data.materia,data.comentario,data.calificacion], (err, values) => {
...
controller.modificarcomentario = (req, res) => { 
    const {ID_Docente} = req.params;

    req.getConnection((err, conn) => {

        conn.query('select * from comentarios where ID_Docente = ? and ID_Usuario = ?', [ID_Docente,req.cookies.usuario] , (err, rows) => {
            res.render('modificarcomentario',{
            data: rows
        });
     });
    })
...
controller.updatecomentario = (req, res) => {
    ...
    req.getConnection((err, conn) => {
        ...
        conn.query('UPDATE comentarios set calificacion = ?, comentario = ?  WHERE ID_Usuario = ?',[data.calificacion, data.comentario, req.cookies.usuario],(err, rows) => {
            console.log(err);
            res.redirect('/perfildocentes/'+ID_Docente);
...
controller.ingresar = (req, res) => {
    ...
    req.getConnection((err, conn) => {
        ...
        conn.query('SELECT ID_Usuario from usuarios where Correo_Institucional_Usuario = ? and Contraseña_Usuario = ?',[datos.email,datos.password], (err, values) => {
            ...
            if (values.length > 0) {
                res.clearCookie('usuario');
                res.cookie("usuario",values[0].ID_Usuario);
                res.redirect('/menu');
